import { FaRegComment, FaSpider } from "react-icons/fa";
import { BiRepost } from "react-icons/bi";
import { FaRegHeart } from "react-icons/fa";
import { FaHandsAslInterpreting, FaRegBookmark } from "react-icons/fa6";
...
const Post = ({ post }) => {
  ...
  const { mutate: deletePost, isPending: isDeleting } = useMutation({
    ...
    onSuccess: () => {
      ...
      //invalidate the query to fetch the posts
  ...
  const { mutate: postComment, isPending: isCommenting } = useMutation({
    mutationFn: async (text) => {
      try {
        ...
        if (!res.ok) throw new Error(data.error || "Something went wrong");
        // console.log("got in frontend", data);
        return data;
      } catch (error) {
        throw new Error(error);
      }
    ...
    onSuccess: (updatedComments) => {
      ...
      queryClient.setQueryData(["posts"], (oldData) => {
        const x = oldData.map((p) => {
          if (p._id === post._id) return { ...post, comments: updatedComments };
          return p;
        });
        // console.log("from frontend", x);
        return x;
      });
    },
    onError: (error) => {
