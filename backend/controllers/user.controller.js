...
// 3) follow -> unfollow si unfollow->follow
// 3.1) follow: $push in followers si following
...
const followUnfollowUser = async (req, res) => {
  try {
    ...
    if (isFollowing) {
      ...
      // TODO: return the id of the user as a response (for the frontend)
...
const getSuggestedUsers = async (req, res) => {
  try {
    ...
    let usersFollowedByMe = await User.findById(userId).select("following");
...
const updateUser = async (req, res) => {
  ...
  try {
    ...
    if (profileImg) {
      if (user.profileImg) {
        // if he has a currectProfileImg you first have to delete it from cloudinary
        // https://res.cloudinary.com/dyfqon1v6/image/upload/v1712997552/zmxorcxexpdbh8b0bkjb.png
        // eu vreau id = zmxorcxexpdbh8b0bkjb
    ...
    // update it or keep the default one
